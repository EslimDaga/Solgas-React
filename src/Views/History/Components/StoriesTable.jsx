...
const StoriesTable = ({ events, loading, onDetailClick }) => {
  ...
  const segundos = Math.round(
    consolaSeleccionada.duration_time % 0x3c
  ).toString();
  const minutos = (
    Math.floor(consolaSeleccionada.duration_time / 0x3c) % 0x3c
  ).toString();

  const segundos_2 = Math.round(
    consolaSeleccionada.game_time % 0x3c
  ).toString();
  const minutos_2 = (
    Math.floor(consolaSeleccionada.game_time / 0x3c) % 0x3c
  ).toString();
          ...
          <TableBody>
            ...
            {events.map((event) => (
              <TableRow key={event.id}>
                ...
                <TableCell>
                  <button
                    onClick={() => seleccionarConsola(event, "Editar")}
            ...
            {!events.length && (
              <TableRow>
                <TableCell colSpan="7">
                  <Empty title="Sin registros. Ingrese la placa y la fecha de creación para obtener eventos" />
                </TableCell>
              </TableRow>
            )}
          </TableBody>
